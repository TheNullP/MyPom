...
async function sessionExpiredDueToTimeout() {
...
function alterarTempo(min) {
	...
	let segundosAdicionais = min * 60;

	if (!savedEndTime) {
		let novoTempo = timeLeft + segundosAdicionais;
		if (novoTempo > 0) {
			timeLeft = novoTempo;
	...
	if (newEndTime > new Date().getTime()) {
		...
		tempoInicial += segundosAdicionais;
		localStorage.setItem("pomoTempoInicial", tempoInicial);
